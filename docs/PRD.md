...
- **中文版**：正文和标题使用中文；品牌名可使用英文名（`GRADO Contract`）或中文名（`格度商业家具`），但两者不得并排出现（如"GRADO Contract 格度商业家具"此类写法不允许）（如"GRADO Contract 格拉多"此类写法不允许出现）；变量填充后的内容（如客户姓名、公司名等）保持原始格式，不强制转换。
