...
标题由 LLM 在模板导入阶段生成，输出为含少量变量的 `subject_template` 字符串，写入 `config.yaml`。发送时由 `email_generator.py` 完成变量替换，生成每封邮件的最终标题。

**生成原则**：

- **来源**：若原始模板中包含明确的邮件标题，LLM 直接提取并转化为含变量的模板；若无标题，LLM 根据正文核心内容自动生成。
- **变量引入**：标题中可引入以下变量，使标题与客户产生自然关联，但不强制全部使用，以自然流畅为优先：

| 可用变量 | 说明 | 示例效果 |
| :--- | :--- | :--- |
| `{{CUSTOMER_COMPANY}}` | 客户公司名 | "...for Gensler" |
| `{{CUSTOMER_FIRST_NAME}}` | 客户名字 | "Hi John, ..." |
| `{{CUSTOMER_INDUSTRY}}` | 客户所在行业 | "...for Financial Spaces" |
| `{{SENDER_MARKET_REGION}}` | 发件人负责区域 | "...in Greater China" |

- **自然度要求**：变量应融入标题语义，不得生硬拼接（如 "GRADO Contract + {{CUSTOMER_COMPANY}}" 此类格式不允许）。标题应简洁，通常不超过 10 个词（英文）或 20 个字（中文）。
- **语言规则**：标题语言与正文保持一致，遵循 §3.5 的中英文纯净规则。
- **终端确认**：导入完成后，在终端以高亮颜色展示生成的 `subject_template`，用户可选择接受或手动修改后再确认。
...
- **中文版**：正文和标题使用中文；品牌名可使用英文名（`GRADO Contract`）或中文名（`格度商业家具`），但两者不得并排出现（如"GRADO Contract 格度商业家具"此类写法不允许）；变量填充后的内容（如客户姓名、公司名等）保持原始格式，不强制转换。
