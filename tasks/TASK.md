...
| TASK-D.4 | 在 `prompts/template_import_prompt.md` 中增加语言纯净规则约束：**英文版**所有文字必须全为英文，不得含任何汉字（品牌名统一使用 `GRADO Contract`）；**中文版**正文和标题使用中文，品牌名允许保留英文或中英并列，变量占位符内容不强制转换。 | LLM 生成的 `en_html` 不含汉字；`cn_html` 正文为中文，经终端预览可验证。 |
